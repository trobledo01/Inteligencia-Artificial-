...
class PokemonRecognizer:
    ...
    def train_model(self, epochs=None, lr=None, batch_size=None):
        """Entrena el modelo con hiperparámetros ajustables"""
        print("\n=== INICIANDO ENTRENAMIENTO DEL MODELO ===")
        
        epochs = epochs or self.hyperparams['epochs']
        lr = lr or self.hyperparams['lr']
...
# Visualización de precisión
    ...
    def evaluate_model(self, show_matrix=True):
        """Evalúa el modelo con métricas avanzadas"""
        print("\n=== EVALUANDO MODELO CON MÉTRICAS AVANZADAS ===")
        ...
        print(classification_report(all_labels, all_preds, target_names=list(self.idx_to_name.values())))
        ...
        if show_matrix:
            ...
            sns.heatmap(cm, annot=False, fmt='d', 
                        xticklabels=self.idx_to_name.values(),
                        yticklabels=self.idx_to_name.values())
    ...
    def compare_models(self, history_list, labels):
        """Compara múltiples entrenamientos en gráficos"""
    ...
    def run_camera(self):
        ...
        print("- 'h': Ajustar hiperparámetros")
        print("- 'm': Métricas avanzadas de evaluación")
        print("- 'q': Salir")
        ...
        while True:
            ...
            # Si presiona 'e', evaluar modelo
                ...
                self.hyperparams['lr'] = float(input("Nueva tasa de aprendizaje (ej: 0.001): "))
                self.hyperparams['batch_size'] = int(input("Nuevo tamaño de batch (ej: 16): "))
                self.hyperparams['epochs'] = int(input("Nuevo número de épocas (ej: 10): "))
                print("Hiperparámetros actualizados!")
        
        # Nueva opción 'm' para métricas avanzadas
            elif key == ord('m'):
                self.evaluate_model(show_matrix=True)
